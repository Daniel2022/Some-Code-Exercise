...
class program(object):
	...
	def part(self):
		...
		for key in key_list:
			...
			str_list.insert(new_main_part.index(key),'\n')
			new_main_part = ''.join(str_list)
		#print(key_list)#ex
		#print([new_main_part])#ex
		locate_list = []
		for i in range(len(new_main_part)):
			if new_main_part[i]==';':
				if re.match(r';\s*fi ',new_main_part[i:]) and (not re.match(r';\s*fi \s*:',new_main_part[i:])):
					locate_list.append(i+1)
					continue
				if re.match(r';\s*done ',new_main_part[i:]) and (not re.match(r';\s*done \s*:',new_main_part[i:])):
					locate_list.append(i+1)
					continue
		#print(locate_list)
		new_main_part_list = list(new_main_part)
		counter = 0
		for locate in locate_list:
			new_main_part_list.insert(locate+counter,'\n')
			counter+=1
		new_main_part = ''.join(new_main_part_list)
		print([new_main_part])#ex
		lines = re.split(r';?\s*\n\s*',new_main_part)
		#print(lines)#ex
		lines_list = []
